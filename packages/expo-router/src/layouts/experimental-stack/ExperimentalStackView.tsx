...
function ScreenView({
  ...
  const headerConfigProps = {
    ...
    backButtonHidden: options.headerBackButtonHidden,
  };
...
function useUnsupportedOptionsWarning(
  ...
  React.useEffect(() => {
    ...
    const unsupported = Object.keys(options).filter(
      (key) => !SUPPORTED_OPTION_KEYS.has(key as keyof ExperimentalStackNavigationOptions)
    );
    ...
    console.warn(
      `ExperimentalStack: ignoring unsupported screenOption${unsupported.length > 1 ? 's' : ''} ${unsupported
        ...
        `The new react-native-screens experimental Stack only supports 'title', 'headerShown', 'headerTransparent', and 'headerBackButtonHidden'. ` +
