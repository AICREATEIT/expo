...
describe('ExperimentalStack — option mapping', () => {
  ...
  it('passes headerTransparent and headerBackButtonHidden through', () => {
    renderRouter(
      {
        a: () => null,
        _layout: () => (
          <ExperimentalStack
            screenOptions={{ headerTransparent: true, headerBackButtonHidden: true }}
          />
        ),
      },
      { initialUrl: '/a' }
...
describe('ExperimentalStack — Header child', () => {
  it('warns and ignores <ExperimentalStack.Header> children', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    try {
      ...
      const ignoredWarnings = warnSpy.mock.calls.filter(
        (call) =>
          typeof call[0] === 'string' && call[0].includes('<ExperimentalStack.Header /> is ignored')
      );
      expect(ignoredWarnings.length).toBe(1);

      const unknownChildWarnings = warnSpy.mock.calls.filter(
        (call) => typeof call[0] === 'string' && call[0].includes('Unknown child element')
      );
      expect(unknownChildWarnings.length).toBe(0);
    } finally {
      warnSpy.mockRestore();
    }
  });
